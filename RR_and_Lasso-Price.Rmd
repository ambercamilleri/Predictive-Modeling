...
clean_listings = read.csv("clean_listings_price.csv") #open the .csv file
...
#```
```{r}
x=model.matrix(price~.,clean_listings)[,-1]
...
```{r}
library(glmnet)
#x=model.matrix(price~. -1, data=clean_listings)
#y=clean_listings$price
```
...
and bc we have 72 values of lambda there will be 72 different columns in this prediction matrix
```{r}
